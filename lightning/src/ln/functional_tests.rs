...
fn do_test_fail_back_before_backwards_timeout(post_fail_back_action: PostFailBackAction) {
	...
	expect_pending_htlcs_forwardable_and_htlc_handling_failed!(
		nodes[1],
		vec![HTLCHandlingFailureType::Forward { node_id: Some(node_c_id), channel_id: chan_2.2 }]
	);
	check_added_monitors(&nodes[1], 1);
...
fn do_test_commitment_revoked_fail_backward_exhaustive(
	...
	expect_pending_htlcs_forwardable_and_htlc_handling_failed!(
		nodes[2],
		vec![HTLCHandlingFailureType::Receive { payment_hash: first_payment_hash }]
	);
	check_added_monitors(&nodes[2], 1);
	let updates = get_htlc_update_msgs!(nodes[2], node_b_id);
	assert!(updates.update_add_htlcs.is_empty());
	...
	expect_pending_htlcs_forwardable_and_htlc_handling_failed!(
		nodes[2],
		vec![HTLCHandlingFailureType::Receive { payment_hash: second_payment_hash }]
	);
	check_added_monitors(&nodes[2], 1);
	let updates = get_htlc_update_msgs!(nodes[2], node_b_id);
	assert!(updates.update_add_htlcs.is_empty());
	...
	expect_pending_htlcs_forwardable_and_htlc_handling_failed!(
		nodes[2],
		vec![HTLCHandlingFailureType::Receive { payment_hash: third_payment_hash }]
	);
	check_added_monitors(&nodes[2], 1);
	let updates = get_htlc_update_msgs!(nodes[2], node_b_id);
	assert!(updates.update_add_htlcs.is_empty());
...
pub fn test_peer_disconnected_before_funding_broadcasted() {
	...
	check_closed_event!(&nodes[0], 2, ClosureReason::DisconnectedPeer, true, [node_b_id], 1000000);
	check_closed_event!(&nodes[1], 1, ClosureReason::DisconnectedPeer, false, [node_a_id], 1000000);
}
...
pub fn test_bump_txn_sanitize_tracking_maps() {
	...
	check_closed_broadcast!(nodes[0], true);
	check_added_monitors(&nodes[0], 1);
	check_closed_event!(nodes[0], 1, ClosureReason::CommitmentTxConfirmed, [node_b_id], 1000000);
	let penalty_txn = {
		let mut node_txn = nodes[0].tx_broadcaster.txn_broadcasted.lock().unwrap();
...
pub fn test_pre_lockin_no_chan_closed_update() {
	...
	check_closed_event!(nodes[0], 2, reason, true, [node_b_id], 100000);
}
...
pub fn test_inconsistent_mpp_params() {
	...
	expect_pending_htlcs_forwardable_and_htlc_handling_failed!(
		nodes[2],
		vec![HTLCHandlingFailureType::Forward { node_id: Some(node_d_id), channel_id: chan_2_3.2 }]
	);
	check_added_monitors(&nodes[2], 1);
...
#[xtest(feature = "_externalize_tests")]
pub fn test_claimed_htlcs_not_in_pending_claim_requests() {
	use crate::events::bump_transaction::sync::WalletSourceSync;
	...
	// Route HTLC 1 and 2 from A to B.
	let (preimage_1, payment_hash_1, ..) = route_payment(&nodes[0], &[&nodes[1]], 1_000_000);
	let (_preimage_2, _payment_hash_2, ..) = route_payment(&nodes[0], &[&nodes[1]], 1_000_000);
	...
	let node_b_commit_tx = get_local_commitment_txn!(nodes[1], cid);
	...
	// Confirm HTLC 1 claiming tx on node A.
	let process_bump_event = |node: &Node| {
		let events = node.chain_monitor.chain_monitor.get_and_clear_pending_events();
		assert_eq!(events.len(), 1);
		let bump_event = match &events[0] {
	...
	let bs_htlc_1_claiming_tx = process_bump_event(&nodes[1]);

	mine_transaction(&nodes[0], &bs_htlc_1_claiming_tx);
	// Node A reaches the HTLC timeout height.
	connect_blocks(&nodes[0], TEST_FINAL_CLTV - 1);

	// Check that node A times out only HTLC 2, and does not attempt to timeout the already
	// claimed HTLC 1.
	let timeout_tx = {
	...
	let htlc_already_claimed = bs_htlc_1_claiming_tx.input.iter().any(|htlc|{
		htlc.previous_output == timeout_tx.input[0].previous_output
	});
	assert!(!htlc_already_claimed, "HTLC already claimed by counterparty's HTLC-Success transaction");
	assert_eq!(timeout_tx.input.len(), 1);
